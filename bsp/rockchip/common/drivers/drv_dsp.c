...
static rt_err_t rk_dsp_send_vendor_key(struct rk_dsp *rkdsp, int *result)
{
    ...
    char *key;
    int ret, level, len = 1024;

    vendor_auth = (struct dsp_auth *)rkdsp_malloc(sizeof(struct dsp_auth));
    ...
    {
        HAL_CPUDelayUs(10);
        rt_hw_cpu_dcache_ops(RT_HW_CACHE_INVALIDATE, (void *)vendor_auth, sizeof(struct dsp_auth));
    }
    while (vendor_auth->flag != DSP_AUTH_RESP_FLAG);
    rt_hw_cpu_dcache_ops(RT_HW_CACHE_INVALIDATE, (void *)vendor_auth->key, vendor_auth->len);
