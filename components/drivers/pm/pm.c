 ...
 */
static int _pm_device_suspend(void)
{
    int index;
    int ret = 0;
    ...
    for (index = 0; index < _pm.device_pm_number; index++)
    {
        ...
        {
            ret = _pm.device_pm[index].ops->suspend(_pm.device_pm[index].device);
            if (ret != RT_EOK)
            {
...
unsigned char rt_current_pm_state()
{
	return _pm.current_mode;
}
...
const static struct rt_device_ops _rt_pm_device_ops =
{
    RT_NULL,
    RT_NULL,
    RT_NULL,
